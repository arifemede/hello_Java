...
public class C01DateTime {
    ...
    public static void main(String[] args) {
        ...
        LocalDate myDate = LocalDate.now();  // Obtains the current date from the system clock in the specified time-zone.
        ...
        //How to format Date
        //Example 1:
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("MM/dd/yyyy"); // lover case m is used for minute. so we use M for month
        String formattedMyDate = dtf.format(myDate);
        System.out.println(formattedMyDate);// 07/17/2023

        //Example 2:
        DateTimeFormatter dt = DateTimeFormatter.ofPattern("dd MMM yyyy"); // MMM is for first three char of the month
        String myDateFormatted = dt.format(myDate);
        System.out.println(myDateFormatted);// 17 Jul 2023

        //Example 3:
        DateTimeFormatter d = DateTimeFormatter.ofPattern("dd MMMM yy"); // MMMM is for full name of the month
        String myDateFormat = d.format(myDate.plusMonths(1));
        System.out.println(myDateFormat);// 17 August 23   // yy is for last two digits of the year


        //How to see date in another time zone
        LocalDate dataInJapan = LocalDate.now(ZoneId.of("Asia/Tokyo"));
        System.out.println(dataInJapan);
